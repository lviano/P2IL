...
"""
for env, max_env, min_env in zip(["PongNoFrameskip-v4"], [21.0], [-21.0]):
    fig = plt.figure()
    ...
    """
    with open("results/data_pong.pkl","rb") as f:
    ...
    with open("results/data_pong_iq.pkl","rb") as f:
        dictionaries = pickle.load(f)
    xs=None
    mean=None
    ...
    ms = (np.mean(mean, axis=0) - min_env)/(max_env - min_env)
    std = (np.std(mean, axis=0))/(max_env - min_env)
    ax.plot(np.mean(xs,axis=0)[::1]/100, ms[::1],"-o", color="goldenrod",label="IQ")
    ...
    plt.xlabel("Expert Samples (x 100)",fontsize=30)
