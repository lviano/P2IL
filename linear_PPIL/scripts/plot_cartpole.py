...
for env, max_env, min_env in zip(["CartPole-v1"], [500.0], [20.0]):
    fig = plt.figure()
    ...
    with open("results/data_cartpole.pkl","rb") as f:
        dictionaries = pickle.load(f)
    xs=None
    mean=None
    for j, d in enumerate(dictionaries.values()):
        if xs is None:
    ...
    with open("results/data_cartpole_iq.pkl","rb") as f:
        dictionaries = pickle.load(f)
    xs=None
    mean=None
