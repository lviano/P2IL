...
def save(agent, epoch, args, output_dir='results/'):
    if epoch % args.save_interval == 0:
        ...
        if not os.path.exists(output_dir):
            os.mkdir(output_dir)
        torch.save(f'{output_dir}/{args.agent.name}_{name}')
...
def main(cfg: DictConfig):
    ...
    net = BCNet(env.observation_space.shape[0],env.action_space.n, 64, args)
    net_optimizer = torch.optim.Adam(net.parameters(), lr=1e-3)
    for epoch in range(epochs):
        predicted_actions = net(expert_obs)
        loss = criterion(predicted_actions, expert_action.flatten().long()) #torch.vstack([torch.eye(env.action_space.n)[int(a[0])] for a in expert_action]))
