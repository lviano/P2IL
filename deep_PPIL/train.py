    ...
    for epoch in count():
        ...
        for episode_step in range(EPISODE_STEPS):
            ...
            if learn_steps % args.env.eval_interval == 0:
                ...
                steps_list.append(learn_steps)
                rewards_list.append(returns)
                with open(hydra.utils.to_absolute_path(f"results/{args.env.name}_PPIL_{args.agent.ppil}_{args.seed}.pkl"), "wb") as f:
                    pickle.dump({"steps":steps_list,"rewards":rewards_list}, f)
...
def iq_update_critic(self, policy_batch, expert_batch, logger, step):
    ...
    if "DoubleQCritic" in self.args.q_net._target_:
        ...
        if not args.agent.ppil:
            ...
            if args.agent.alternate:
                current_r1, current_r2 = self.get_reward(obs, action, both=True)
                q1_loss, loss_dict1 = alternate_ppil_loss(agent, current_r1, current_Q1, current_V, next_V, batch, w_update)
                q2_loss, loss_dict2 = alternate_ppil_loss(agent, current_r2, current_Q2, current_V, next_V, batch,w_update)
                        
            else:
                current_r1, current_r2 = self.get_reward(obs, action, both=True)
                q1_loss, loss_dict1 = ppil_loss(agent, current_r1, current_Q1, current_V, next_V, batch)
                q2_loss, loss_dict2 = ppil_loss(agent, current_r2, current_Q2, current_V, next_V, batch)
        ...
        if not args.agent.ppil:
            ...
            if args.agent.alternate:
                critic_loss, loss_dict = alternate_ppil_loss(agent, current_r, current_Q, current_V, next_V, batch, w_update)
                
            else:
                critic_loss, loss_dict = ppil_loss(agent, current_r, current_Q, current_V, next_V, batch)
    ...
    if args.agent.alternate:
        ...
        # Optimize the critic
        self.critic_optimizer.zero_grad()
        if args.agent.ppil: self.reward_optimizer.zero_grad()
        critic_loss.backward()
        # step critic
        self.critic_optimizer.step()
        if args.agent.ppil: self.reward_optimizer.step()

    w_update = not w_update 
    ...
    if self.actor and step % self.actor_update_frequency == 0:
        if not self.args.agent.vdice_actor:
            ...
            if self.args.offline:
                ...
                if self.args.proportion_expert==1:
                    obs = torch.cat([policy_batch[0], expert_batch[0]], dim=0)
                else:
                    f = int(len(expert_batch[0])*self.args.proportion_expert)
                    obs = torch.cat([policy_batch[0], expert_batch[0][:f]], dim=0)
